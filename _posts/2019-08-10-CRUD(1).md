...
## URL 설정
프로젝트 이름과 똑같은 폴더에 `urls.py`에 주소를 설정한다.
...
app_name을 사용해서 주소의 이름을 지정했다. rails의 컨트롤러 기능과 유사한 것 같다.
)
...
`new.html`에 입력을 위핸 form 을 작성했다.
~~~html
<h2>
    새로 만들기
</h2>

<form action="{% raw %}{% url 'crud:create' %}{% endraw %}" method="POST">
